...
export async function bookmark(videoId, videoTitle) {
  try {
      ...
      const video = searchPosts(videoTitle);
      console.log("VIDEO" + video);
      
      console.log("B4 UPDATE: ", currentUser.bookmarkedVideos);

      const videoUrls = [];

      let videos = currentUser.bookmarkedVideos.forEach(element => {
        console.log("PUSHING " +element.$id);
        videoUrls.push(element.$id);
      });
      ...
      const updatedDocument = {
        ...currentUser,
        bookmarkedVideos: videoUrls
      }
    
      console.log("UPDATED: " + typeof updatedDocument.bookmarkedVideos + " LENGTH: " + updatedDocument.bookmarkedVideos.length);
