...
export async function bookmark(user, videoId) {
  try {
      databases.updateDocument(
        databaseId,
        userCollectionId,
        user.$id,
        { bookmarkedVideos: [...user.bookmarkedVideos, videoId] }
      )
  } catch (error) {
    throw new Error(error);
  }
...
/**
 * @param {object} user - User in type (Models.Document)
 * @param {string} videoId - ID of the video in the database
 */
export async function unbookmark(user, videoId) {
  try {
    const targetIndex = user.bookmarkedVideos.findIndex((video) => {
      if (video.$id == videoId) {
        return video
      }
    })
    const updatedVideos = user.bookmarkedVideos
    const removedVideo = updatedVideos.splice(targetIndex, 1)[0];

    databases.updateDocument(
      databaseId,
      userCollectionId,
      user.$id,
      { bookmarkedVideos: updatedVideos }
    );
