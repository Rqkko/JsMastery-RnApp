...
export async function getBookmarkedPosts(user) {
  try {
    ...
    console.log("BOOKMARK " + user.bookmarkedVideos);
    let posts = []
    for (const video of user.bookmarkedVideos) {
      console.log("Getting post with ID: " + video.$id);
      const post = await getPostWithId(video.$id);
      posts.push(post);
    }
    // TODO: Try using Promise.all w/ .map?
